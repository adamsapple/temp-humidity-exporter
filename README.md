...
- `src/thexporter.py`: Flask アプリ本体と BLE スキャナ
...
- `./config.json` を `/app/config.json` としてコンテナへ読み込み
...
温湿度計ごとに BLE 広告フォーマットは異なります。もし実機が `bthome` / `pvvx_custom` 以外の形式を使っている場合は、その広告データ仕様に合わせて `src/thexporter.py` の decoder を追加してください。
